...
suffixs = pd.read_csv(os.path.abspath(os.path.join(script_dir, '../../_static/mapping_file/suffix.csv')))
...
def get_entity_names(company_list_name: str, econ_list_name: str,
                     logger, include_companies: bool=True, include_econs: bool=True) -> List[str]:
    """
    ...
    if include_companies:
        ...
        comp_df = pd.read_csv(os.path.abspath(os.path.join(script_dir, f'../../_static/mapping_file/{company_list_name}')), index_col=0)
    ...
    if include_econs:
        ...
        econ_infor = pd.read_csv(os.path.abspath(os.path.join(script_dir, f'../../_static/mapping_file/{econ_list_name}')))
        econ_infor = econ_infor[
            [econ_infor.columns[0], econ_infor.columns[1]]]
        econ_infor.columns = ["econ_name", "econ_id"]
        econ_list = []
        econ_id = []
        for i in range(econ_infor.shape[0]):
            if econ_infor.isna()["econ_name"][i] == False:
...
def split_iter(list1: list, batch_num: int) -> list:
...
def insert_mapped_dict(col, **kwargs):
    required_fields = ['match_u3id', 'match_company', 'ner_comp_similarity2', 'ner_comp', 'match_similarity',
                       'Similarity_1st', 'Similarity_2nd', 'cleaned_ner_entity_1st', 'matched_cleaned_comp_1st',
                       'ner_entity_2nd', 'matched_comp_2nd']

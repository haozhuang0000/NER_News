...
class SimilarityMapping(MongoDBHandler, MilvusDB):
    ...
    def __init__(self, in_col: str, out_col: str,
                 ner_mapped_col: str='ner_mapped', inserted_threshold: int=1000, embeddings: Optional[Union[NVEmbed, str]] = None) -> None:
        """
        ...
        self.combined_full_comp_list, self.econ_list, self.econ_id = \
            get_entity_names(company_list_name=os.environ['COMPANY_PATH_NAME'],
                             econ_list_name=os.environ['ECON_PATH_NAME'],
                             logger=self.logger)
        self.full_name_dict = {number: name for name, number in self.combined_full_comp_list}
    ...
    def transform_data_to_results_dict(self, find_cursor: list) -> dict:
        ...
        """
        results_dict = {"_id": [], "Sentence_id": [], "Output_sentence1": [], "Output_sentence2": [],
    ...
    def calculate_similarity_helper(self, input_article: dict,
                                    ner_comp: str,
                                    ner_comp_original: str,
                                    lower_bound: int,
                                    _id: str,
                                    check: str,
                                    result_list: list) -> tuple[list, bool]:
        """
        ...
        """
        def _compute_similarity_for_each_mode(ner_comp: str, check: str,
                                              name_to_map: str, name_to_map_type: str) -> Optional[int]:

            checker = {'Full_Name': 'full',
                       'Common_Name': 'common',
                       'Clean_Name': 'clean',
            ...
            if check_type == name_to_map_type or name_to_map_type == 'ticker':
                if first_word(ner_comp.lower()) != first_word(name_to_map.lower()):
                    ...
                    similarity = Levenshtein.ratio(ner_comp.lower(), name_to_map.lower())
                    return similarity
            else:
                return None
        ...
        if mapped_NER_Name != []:
            ...
            for idx in range(0, len(result_list)):
                ...
                similarity = _compute_similarity_for_each_mode(ner_comp, check, name_to_map, name_to_map_type)
                ...
                if similarity >= lower_bound:
                    ...
                    matched_cleaned_comp_1st.append(self.full_name_dict[result_list[idx]['u3_id']])
            ...
            if flag == True:
                ...
                if flag == False:
                    insert_mapped_dict(col=self.ner_mapping_col,
    ...
    def calculate_similarity(self, input_article: dict, lower_bound: int) -> dict:
        ...
        Parameters:
        - input_article (dict): A dictionary containing details about the article. Expected keys include:
            - 'Companies_econs': List of company names and country names extracted from the article.
            - others
        - lower_bound (int): A threshold value used to filter out matches based on similarity score.
        ...
        for q in range(len(input_article["Companies_econs"])):
            if len(input_article['Output_sentence2'][q]) != 0:
                if input_article['Output_sentence2'][q][8] == 'C':
                    ...
                    results = self.vectorsearch(ner_company_name=ner_comp, embeddings=self.embeddings)
                    result_list_full = []
                    for i in results[0]:
                        if i['distance'] > 1:
                    ...
                    input_article, flag = self.calculate_similarity_helper(input_article, ner_comp,
                                                                      ner_comp_similarity2, lower_bound, _id,
                                                                      check='Full_Name', result_list=result_list_full)
                    if flag == True:
                        ########### ------------------ 2. Comparing company name in the article with common name in the list ------------------ #############
                        input_article, flag = self.calculate_similarity_helper(input_article, ner_comp,
                                                                          ner_comp_similarity2, lower_bound, _id,
                                                                          check='Common_Name', result_list=result_list_full)

                        if flag == True:
                            ########### ------------------ 3 Comparing company name in the article with clean name in the list ------------------ #############
                            input_article, flag = self.calculate_similarity_helper(input_article, ner_comp,
                                                                              ner_comp_similarity2, lower_bound, _id,
                                                                              check='Clean_Name', result_list=result_list_full)
                            if flag == True:
                                ...
                                results = self.vectorsearch(ner_company_name=cleaned_len1_ner_entity, embeddings=self.embeddings)
                                result_list_clean = []
                                for i in results[0]:
                                    if i['distance'] > 1:
                                ...
                                input_article, flag = self.calculate_similarity_helper(input_article,
                                                                                  cleaned_len1_ner_entity,
                                                                                  ner_comp_similarity2, lower_bound, _id,
                                                                                  check='Clean_Name', result_list=result_list_clean)
                                if flag == True:
                                    ########### ------------------ 5.1 clean with full suffix ------------------ #############
                                    cleaned_ner_entity = clean_string(cleaned_len1_ner_entity)
                                    ...
                                    results = self.vectorsearch(ner_company_name=cleaned_ner_entity,
                                    ...
                                    input_article, flag = self.calculate_similarity_helper(input_article, cleaned_ner_entity,
                                                                                      ner_comp_similarity2, lower_bound,
                                                                                      _id,
                                                                                      check='Clean_Name_S', result_list=result_list_clean_s)
                ...
                if input_article['Output_sentence2'][q][8] == 'E':
                    ...
                    for idx in range(len(self.econ_list)):
                        similarity = Levenshtein.ratio(input_article['Companies_econs'][q],
                                                       self.econ_list[idx])
                        if similarity > lower_bound:
                            input_article['Entity_id'].append(int(int(self.econ_id[idx])))
...
if __name__ == '__main__':
    ...
    similarity_map = SimilarityMapping(in_col=args.in_col, out_col=args.out_col, embeddings=embeddings)
    similarity_map.run()
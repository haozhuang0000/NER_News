...
def prepare_data(embeddings):
    ...
    cleancomp_df["companyid_type"] = cleancomp_df.CompanyID.apply(
        lambda x: str(x) + "_clean"
    )
    ...
    cleancomp_s_df["companyid_type"] = cleancomp_s_df.CompanyID.apply(
        lambda x: str(x) + "_clean_s"
    )
    ...
    common_df["companyid_type"] = common_df.CompanyID.apply(
        lambda x: str(x) + "_common"
    )
    ...
    df_cleaned["companyid_type"] = df_cleaned.CompanyID.apply(
        lambda x: str(x) + "_ticker"
    )
...
def creaet_NER_Mapping(embeddings, vdb_name):
    ...
    fields = [
        ...
        FieldSchema(
            name="companyid", dtype=DataType.DOUBLE, auto_id=False, max_length=100
        ),
    ...
    col_ = milvusdb._connect_vdb(
        vdb_name, description="NER_Mapping_Companies", fields=fields
    )
    ...
    (
        ...
        com_name_entities,
        tic_entities,
    ) = prepare_data(embeddings=embeddings)
